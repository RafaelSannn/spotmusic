...
public class MusicaController {
    ...
    @GetMapping("/cadastro")
    public String preSalvar(@ModelAttribute("musica") Musica musica, @PathVariable("playlistId") long playlistId, ModelMap model) {
        model.addAttribute("playlistId", playlistId);
        return "musica/add";
    }

    @PostMapping("/salvar")
    public String salvar(@PathVariable("playlistId") long playlistId, @Valid @ModelAttribute("musica") Musica musica, BindingResult result, RedirectAttributes attr, ModelMap model) {
        if (result.hasErrors()) {
            model.addAttribute("playlistId", playlistId);
            return "musica/add";
        }
    ...
    @PutMapping("/salvar")
    public String atualizar(@PathVariable("playlistId") long playlistId, @Valid @ModelAttribute("musica") Musica musica, BindingResult result, RedirectAttributes attr, ModelMap model) {
        if (result.hasErrors()) {
            model.addAttribute("playlistId", playlistId);
            return "musica/add";
        }
